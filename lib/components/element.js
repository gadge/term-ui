...
const {
        ATTACH, CLICK, CLOSE, DETACH, FOCUS, KEY, KEYPRESS,
        MOUSE, MOUSEDOWN, MOUSEMOVE, MOUSEOUT, MOUSEOVER, MOUSEUP, MOUSEWHEEL,
...
Element.prototype.config = function (options) {
  ...
  if (options.position.width === 'shrink' || options.position.height === 'shrink') {
    if (options.position.width === 'shrink') delete options.position.width
...
Object.defineProperties(Element.prototype, {
  focused: { get() { return this.screen.focused === this } },
  visible: {
    get() {
      let el = this
      do {
  ...
  draggable: {
    get() { return this._draggable === true },
    set(val) { return val ? this.enableDrag(val) : this.disableDrag() }
  },
  width: {
    get() { return this._getWidth(false) },
    set(val) {
  ...
  height: {
    get() { return this._getHeight(false) },
    set(val) {
  ...
  aleft: {
    get() { return this._getLeft(false) },
    set(val) {
      let expr
  ...
  aright: {
    get() { return this._getRight(false) },
    set(val) {
  ...
  abottom: {
    get() { return this._getBottom(false) },
    set(val) {
  ...
  rleft: {
    get() { return this.aleft - this.parent.aleft },
    set(val) {
      if (this.position.left === val) return
      if (/^\d+$/.test(val)) val = +val
      this.emit(MOVE)
  ...
  rright: {
    get() { return this.aright - this.parent.aright },
    set(val) {
      if (this.position.right === val) return
      this.emit(MOVE)
      this.clearPos()
  ...
  rtop: {
    get() { return this.atop - this.parent.atop },
    set(val) {
      if (this.position.top === val) return
      if (/^\d+$/.test(val)) val = +val
      this.emit(MOVE)
  ...
  rbottom: {
    get() { return this.abottom - this.parent.abottom },
    set(val) {
      if (this.position.bottom === val) return
      this.emit(MOVE)
      this.clearPos()
  ...
  ileft: { get() { return (this.border ? 1 : 0) + this.padding.left } },
  itop: { get() { return (this.border ? 1 : 0) + this.padding.top } },
  iright: { get() { return (this.border ? 1 : 0) + this.padding.right } },
  ibottom: { get() { return (this.border ? 1 : 0) + this.padding.bottom } },
  iwidth: { get() { return (this.border ? 2 : 0) + this.padding.left + this.padding.right } },
  iheight: { get() { return (this.border ? 2 : 0) + this.padding.top + this.padding.bottom } },
  tpadding: { get() { return this.padding.left + this.padding.top + this.padding.right + this.padding.bottom } },
  left: { get() { return this.rleft }, set(val) { return this.rleft = val } },
  right: { get() { return this.rright }, set(val) { return this.rright = val } },
  top: { get() { return this.rtop }, set(val) { return this.rtop = val } },
  bottom: { get() { return this.rbottom }, set(val) { return this.rbottom = val } },
})
    ...
    for (let i = 0; i < line.length; i++) {
      if (line[i] === '\x1b') {
        if ((c = /^\x1b\[[\d;]*m/.exec(line.substring(i)))) {
          attr = this.screen.attrCode(c[0], attr, normAttr)
          i += c[0].length - 1
        }
      }
    }
