...
Program.prototype.log = function () { return this._log('LOG', util.format.apply(util, arguments)) }

Program.prototype.debug = function () { if (this.options.debug) { return this._log('DEBUG', util.format.apply(util, arguments)) } }

Program.prototype._log = function (pre, msg) { if (this._logger) { return this._logger.write(pre + ': ' + msg + '\n-\n') } }
...
Program.prototype.has = function (name) { return this.tput ? this.tput.has(name) : false }

Program.prototype.term = function (is) { return this.terminal.indexOf(is) === 0 }
...
Program.prototype.out = function (name, ...args) {
  this.ret = true
...
Program.prototype._owrite =
  Program.prototype.write =
    Program.prototype.ow = function (text) { if (this.output.writable) { return this.output.write(text) } }
...
Program.prototype._write =
  Program.prototype.wr = function (text) { return this.ret ? text : this.useBuffer ? this._buffer(text) : this.ow(text) }
Program.prototype._twrite =
  Program.prototype.tw = function (data) {
    const self = this
    let iterations = 0,
        timer
    if (this.tmux) {
...
Program.prototype.echo =
  Program.prototype.print = function (text, attr) { this.wr(attr ? this.text(text, attr) : text) }
 ...
 * Helpers
 */
function merge(target, ...sources) { return sources.forEach(o => Object.keys(o).forEach(key => target[key] = o[key])), target }
